...
class TestSlice(unittest.TestCase):
    def test_calculate_scale_and_shift(self):
        ...
        self.assertEqual([0.5, 0.5, 0.5], scale.tolist())
        self.assertEqual([-0.5, -0.5, 0], shift.tolist())
        self.assertEqual([11, 11, 10], res.tolist())

        scale, shift, res = slice.calculate_scale_and_shift(np.array([0, 0, 0]), np.array([20.5, 20.5, 20]), 20, None)
        self.assertEqual(scale.tolist(), [1, 1, 1])
        self.assertEqual(shift.tolist(), [-0.25 ,-0.25 , 0 ])
        self.assertEqual(res.tolist(), [21, 21, 20])

        scale, shift, res = slice.calculate_scale_and_shift(np.array([0, 0, 0]), np.array([20.125, 20.125, 20]), 10, None)
        self.assertEqual(scale.tolist(), [0.5, 0.5, 0.5])
        self.assertEqual(shift.tolist(), [-0.9375, -0.9375, 0.0])
        self.assertEqual(res.tolist(), [11, 11, 10])
