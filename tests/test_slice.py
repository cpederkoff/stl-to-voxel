...
class TestSlice(unittest.TestCase):
    def test_calculate_scale_and_shift(self):
        scale, shift, res = slice.calculate_scale_and_shift(
            np.array([-2, -2, -2]), np.array([2, 2, 2]), None, 1.5
        )
        ...
        scale, shift, res = slice.calculate_scale_and_shift(
            np.array([0, 0, 0]), np.array([4, 4, 4]), None, 1.5
        )
        np.testing.assert_array_equal([2 / 3, 2 / 3, 2 / 3], scale)
        ...
        scale, shift, res = slice.calculate_scale_and_shift(
            np.array([0, 0, 0]), np.array([2, 2, 2]), None, 1
        )
        ...
        scale, shift, res = slice.calculate_scale_and_shift(
            np.array([0, 0, 0]), np.array([2, 2, 2]), 1, None
        )
        ...
        scale, shift, res = slice.calculate_scale_and_shift(
            np.array([0, 0, 0]), np.array([21, 21, 20]), 10, None
        )
        np.testing.assert_array_equal([0.5, 0.5, 0.5], scale)
        ...
        scale, shift, res = slice.calculate_scale_and_shift(
            np.array([0, 0, 0]), np.array([20.5, 20.5, 20]), 20, None
        )
        np.testing.assert_array_equal([1, 1, 1], scale)
        np.testing.assert_array_equal([0.25, 0.25, 0.5], shift)
        np.testing.assert_array_equal([21, 21, 20], res)

        scale, shift, res = slice.calculate_scale_and_shift(
            np.array([0, 0, 0]), np.array([20.125, 20.125, 20]), 10, None
        )
        np.testing.assert_array_equal([0.5, 0.5, 0.5], scale)
        ...
        scale, shift, res = slice.calculate_scale_and_shift(
            np.array([-150, -150, -150]), np.array([150, 150, 150]), 3, None
        )
    ...
    def test_where_line_crosses_z(self):
        ...
        np.testing.assert_array_equal(
            slice.where_line_crosses_z(p1, p2, 3), np.array([1.5, 3, 3])
        )
        np.testing.assert_array_equal(
    ...
    def test_linear_interpolation(self):
        ...
        np.testing.assert_array_equal(
            slice.linear_interpolation(p1, p2, 0.5), np.array([1.5, 3, 3])
        )

    def test_triangle_to_intersecting_lines(self):
        tri = np.array([[2, 4, 1], [1, 2, 5], [3, 2, 3]])
        expected = np.array(
            [
                [2.0, 2.0, 4.0],
                [1.25, 2.5, 4.0],
        ...
        np.testing.assert_array_equal(
            slice.triangle_to_intersecting_points(tri, 4), expected
        )

    def test_triangle_to_intersecting_lines_one_point_same(self):
        tri = np.array([[2, 4, 1], [1, 2, 5], [3, 2, 3]])
        expected = np.array([tri[2], [1.5, 3, 3]])
        np.testing.assert_array_equal(
            slice.triangle_to_intersecting_points(tri, 3), expected
        )
    ...
    def test_triangle_to_intersecting_lines_intersect_one_point(self):
        ...
        np.testing.assert_array_equal(
            slice.triangle_to_intersecting_points(tri, 5), expected
        )
